            ...
            >
                {slideData.map((slide, index) => (
                    <SwiperSlide key={index}>
                        <div className="relative w-full aspect-[2/3] rounded-lg md:rounded-xl lg:rounded-2xl 2xl:rounded-3xl overflow-hidden" onClick={() => {
                            router.push(
                                `/${slide.link}/ID ${slide.id}?title=${encodeURIComponent(slide.title)}&date=${encodeURIComponent(slide.date)}&location=${encodeURIComponent(slide.location)}`
                            );
                        }}
                        >
                        ...
                        <div className="absolute z-10 bottom-0 text-white flex flex-row-reverse justify-between items-center w-full p-1 ">
                            <div className="flex flex-col gap-0.5 items-end p-0 sm:p-1">
                                <p className="text-[8px] sm:text-[10px] md:text-[10px] lg:text-[16px]  2xl:text-[18px]">{slide.city}</p>
                                <p className="text-[6px] sm:text-[8px] md:text-[8px] lg:text-[12px] 2xl:text-[16px]">{slide.date}</p>
                            </div>
